...
export default function Admin() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/auth/api/admin/stats', {
  ...
  const fetchWinners = useCallback(async () => {
    try {
      const token = useAuthStore.getState().token;
      const res = await fetch('http://localhost:5000/api/auth/api/admin/winners', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleCreateQuiz = async (e: React.FormEvent) => {
    ...
    try {
      const token = useAuthStore.getState().token;
      const res = await fetch('http://localhost:5000/api/auth/api/admin/quizzes', {
        method: 'POST',
        headers: { 
  ...
  const handleCreateTip = async (e: React.FormEvent) => {
    ...
    try {
      const token = useAuthStore.getState().token;
      const res = await fetch('http://localhost:5000/api/auth/api/admin/tips', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
