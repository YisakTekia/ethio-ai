...
export default function Education() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: 'ሰላም! እኔ የ Star Think የትምህርት AI ነኝ። የትምህርት ጥያቄዎች፣ የጥናት ስልቶች ወይም አዳዲስ ነገሮችን ለመማር ዝግጁ ነኝ። ምን ልርዳዎት?',
      isBot: true,
    }
  ]);
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6 pb-28 scrollbar-hide">
        ...
        <div className="bg-indigo-50/80 backdrop-blur-sm border border-indigo-100 rounded-2xl p-4 mx-auto max-w-[90%] text-center shadow-sm">
          <p className="text-xs text-indigo-800 font-medium leading-relaxed">
            ...
            <span className="font-bold">ትምህርት ማሳሰቢያ፡</span> ይህ AI ለጥናት እና ለእውቀት ማዳበሪያነት የሚያገለግል ነው።
          </p>
        </div>
          ...
          <div key={msg.id} className={`flex ${msg.isBot ? 'justify-start' : 'justify-end'} animate-fade-in-up`}>
            <div className={`flex max-w-[85%] gap-2.5 ${msg.isBot ? 'flex-row' : 'flex-row-reverse'}`}>
              <div className="flex-shrink-0 mt-auto mb-1">
                {msg.isBot ? (
                  <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-600 to-violet-600 flex items-center justify-center shadow-md">
              ...
              <div className={`p-4 rounded-3xl shadow-sm text-[15px] leading-relaxed ${
                msg.isError 
                  ...
                  : msg.isBot 
                    ...
                    : 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-br-sm shadow-indigo-200'
              }`}>
