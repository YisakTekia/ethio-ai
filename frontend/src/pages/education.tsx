    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/chat', { 
        ...
        body: JSON.stringify({
          ...
          domain: 'education' // <--- CRITICAL: Tells backend to use Education strict context
        }),
      });
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] relative">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-4 space-y-4 pb-20 scrollbar-hide">
        ...
        <div className="bg-indigo-50/80 backdrop-blur-sm border border-indigo-100 rounded-xl p-3 mx-4 mb-6 text-center shadow-sm">
          <p className="text-xs text-indigo-700 font-medium leading-relaxed">
            <span className="font-bold">ማሳሰቢያ፡</span> ይህ AI የትምህርት ኤክስፐርት ብቻ ነው። እባክዎ ከ ትምህርት፣ ፈተና እና የጥናት ስልቶች ውጪ ያሉ ጥያቄዎችን ከመጠየቅ ይቆጠቡ።
          </p>
        </div>
          ...
          <div key={msg.id} className={`flex ${msg.isBot ? 'justify-start' : 'justify-end'} animate-fade-in-up`}>
            <div className={`flex max-w-[85%] gap-2 ${msg.isBot ? 'flex-row' : 'flex-row-reverse'}`}>
              ...
              <div className="flex-shrink-0 mt-auto mb-1">
                {msg.isBot ? (
                  <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-500 to-blue-400 flex items-center justify-center shadow-md">
      ...
      <div className="absolute bottom-2 left-0 right-0 bg-white/80 backdrop-blur-xl border border-gray-200/50 p-2 rounded-3xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)]">
        <form onSubmit={handleSendMessage} className="flex items-center gap-2 relative">
          <input
            ...
            placeholder="የትምህርት ጥያቄዎን ይጻፉ..."
            className="flex-1 bg-gray-100 text-gray-800 placeholder-gray-400 px-5 py-3.5 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
          />
          <button
            type="submit"
            ...
            className={`p-3.5 rounded-2xl flex items-center justify-center transition-all duration-300 ${
              !inputText.trim() || isLoading
                ...
                : 'bg-gradient-to-r from-indigo-500 to-blue-500 text-white shadow-lg shadow-indigo-200 hover:scale-105 active:scale-95'
            }`}
          >
