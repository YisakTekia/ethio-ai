...
export default function Health() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: 'ሰላም! እኔ የ Star Think የጤና AI ነኝ። ስለ ጤናማ አመጋገብ፣ የአካል ብቃት እንቅስቃሴ ወይም አጠቃላይ ጤንነት ምን ልምከርዎት?',
      isBot: true,
    }
  ]);
  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      try {
        ...
        const response = await fetch('https://ethio-ai-backend.onrender.com/api/chat/history?domain=health', {
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6 pb-28 scrollbar-hide">
        ...
        <div className="bg-red-50/80 backdrop-blur-sm border border-red-100 rounded-2xl p-4 mx-auto max-w-[90%] text-center shadow-sm">
          <p className="text-xs text-red-800 font-medium leading-relaxed">
            ...
            <span className="font-bold">የጤና ማሳሰቢያ፡</span> ይህ AI አጠቃላይ መረጃን ብቻ ይሰጣል። የህክምና ባለሙያን ምክር አይተካም።
          </p>
        </div>
          ...
          <div key={msg.id} className={`flex ${msg.isBot ? 'justify-start' : 'justify-end'} animate-fade-in-up`}>
            <div className={`flex max-w-[85%] gap-2.5 ${msg.isBot ? 'flex-row' : 'flex-row-reverse'}`}>
              <div className="flex-shrink-0 mt-auto mb-1">
                {msg.isBot ? (
                  <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-red-500 to-rose-600 flex items-center justify-center shadow-md">
              ...
              <div className={`p-4 rounded-3xl shadow-sm text-[15px] leading-relaxed ${
                msg.isError 
                  ...
                  : msg.isBot 
                    ...
                    : 'bg-gradient-to-r from-red-500 to-rose-600 text-white rounded-br-sm shadow-red-200'
              }`}>
          ...
          <button
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 flex-shrink-0 ${
              !inputText.trim() || isLoading
                ...
                : 'bg-red-600 text-white shadow-lg shadow-red-200 hover:scale-105 active:scale-95'
            }`}
          >
