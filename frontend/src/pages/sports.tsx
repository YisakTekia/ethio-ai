...
export default function Sports() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: 'ሰላም! እኔ የ Star Think የስፖርት እና መዝናኛ AI ነኝ። ስለ እግር ኳስ፣ አትሌቲክስ፣ ፊልሞች ወይም ሙዚቃ ምን ልንገርዎት?',
      isBot: true,
    }
  ]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/auth/api/chat', { 
        method: 'POST',
        headers: { 
      ...
      setMessages((prev) => [...prev, newBotMsg]);
       ...
       // UI/UX Graceful Error Handling: Display friendly Amharic messages instead of raw codes
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] relative bg-slate-50/50">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6 pb-28 scrollbar-hide">
        ...
        {/* Star Think Disclaimer Banner */}
        <div className="bg-white/80 backdrop-blur-sm border border-blue-100 rounded-2xl p-4 mx-auto max-w-[90%] text-center shadow-sm">
          <p className="text-xs text-blue-800 font-medium leading-relaxed">
            <Sparkles className="inline-block w-4 h-4 text-blue-500 mr-1 mb-0.5" />
            <span className="font-bold">Star Think ማሳሰቢያ፡</span> ይህ AI የስፖርት እና መዝናኛ መረጃዎችን ብቻ ነው የሚሰጠው።
          </p>
        </div>
              ...
              <div className={`p-4 rounded-3xl shadow-sm text-[15px] leading-relaxed ${
                msg.isError 
                  ...
                  : msg.isBot 
                    ...
                    : 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-br-sm shadow-blue-200' // User message styling
              }`}>
